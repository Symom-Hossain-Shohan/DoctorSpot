...
<?php
  session_start();
  $kity = $_SESSION['city']; 
  $hosname = $_SESSION['hosname']; 

    $con = mysqli_connect("localhost", "root", "", "ipproject") or die("Couldn't connect");
    $sql1 = "SELECT * FROM hospital WHERE location =  '$kity'";
  ...
  // else if($hosname=="")
  // {
  //   if($kity =="Dhaka" || $kity =="Chittagong" || $kity =="Khulna" || $kity =="Rajshahi" || $kity =="Barishal" || $kity =="Sylhet")  
  //   {
  //     $con = mysqli_connect("localhost", "root", "", "ipproject") or die("Couldn't connect");
  //     $sql1 = "SELECT * FROM hospital WHERE location =  '$kity'";
  //   }
  ...
  // {
  //   $con = mysqli_connect("localhost", "root", "", "ipproject") or die("Couldn't connect");
  //   $sql1 = "SELECT * FROM hospital WHERE name =  '$hosname'";
  // }


  if($hosname === "Search Your Hospital" && $kity === "")
  {
    $con = mysqli_connect("localhost", "root", "", "ipproject") or die("Couldn't connect");
    $sql1 = "SELECT * FROM hospital WHERE '$nam' ";
    $all_docs = $con->query($sql1);
  ...
  else 
  {
    ...
    else 
    {
      ...
      {
        $con = mysqli_connect("localhost", "root", "", "ipproject") or die("Couldn't connect");
        $sql1 = "SELECT * FROM hospital WHERE  name LIKE '%$hosname%' ";
        $all_docs = $con->query($sql1);
      }
      else 
      {
        $con = mysqli_connect("localhost", "root", "", "ipproject") or die("Couldn't connect");
        $sql1 = "SELECT * FROM hospital WHERE location ='$kity' AND name LIKE '%$hosname%' ";
        $all_docs = $con->query($sql1);
      }
    }
  }
 ...
 -->
  ...
  <div class="Hcontainer">
    <div class="search-bar">
        ...
        <input id="inputfield"  placeholder ="Search Your Hospital">
    </div>
